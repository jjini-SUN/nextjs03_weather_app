...
export default function Gpt({ answer, setAnswer }: Props) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    event.preventDefault(); // form 태그 새로고침을 막는다
    if (!inputValue) return; // 입력값 없으면 return해서 그냥 끝내줌
    ...
    try {
      ...
      const response = await fetch("api/gpt", {
        ...
        //object를 JSON 문자열로 바꿔서 서버에 전송한다
      });
    ...
    <div className="gpt-container">
      ...
      <form onSubmit={handleSubmit}>
        <input
          ...
          value={inputValue}
        />
        <button type="submit" className="gpt-button">
          입력
        </button>
      </form>
